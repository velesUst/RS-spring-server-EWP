package com.ewp.server.persistence.repository;
...
public class HibernateRepositoryImpl<T> implements HibernateRepository<T> {
    ...
    @PersistenceContext
    private EntityManager entityManager;  // @PersistenceContext takes care to create a unique EntityManager for every thread. In a production application you can have multiple clients calling your application in the same time. For each call, the application creates a thread. Each thread should use its own EntityManager.
